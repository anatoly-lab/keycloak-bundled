...
9. Malformed `resource` (with a fragment) → `/authorize` returns 400 `invalid_target` before the mapper runs (`ResourceIndicatorValidation.java:14-38`), confirming we inherit that validation for free.
