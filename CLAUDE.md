...
**Keycloak version this repo tracks:** `26.7.2`. The consumer's Keycloak Operator is bumped to `26.7.2` in lockstep (this image is published first, then the operator follows — see the coordination rule below). The earlier `26.5.7` hold — motivated by `adorsys/keycloak-config-cli` lacking a `6.5.0-26.6.x` build — no longer blocks version bumps: config-cli stays pinned at its `6.5.1-26.5.5` tag and reconciles a newer server cleanly so long as the consumer's realm JSON uses none of the newer server's version-only realm fields. Verified for `26.6.4`; **not** re-verified for `26.7.2` — re-check on each bump. This repo tracks the same Keycloak version the consumer deploys.
