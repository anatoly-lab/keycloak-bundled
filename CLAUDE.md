...
**Sole output of this repo:** an OCI image pushed to Harbor.

**Image tag pattern (proposed, confirm with user):** `harbor.anatoly.dev/ankimcp/keycloak:<keycloak-version>-rememberme<plugin-version>` plus `:latest`.
...
anki-mcp-keycloak/
  ...
  .github/workflows/
    build.yml              # GitHub Actions: build image + push to Harbor
...
3. **GitHub Actions workflow** (`.github/workflows/build.yml`):
   ...
   - Steps: checkout → set up buildx → docker login to Harbor (`HARBOR_USERNAME` + `HARBOR_TOKEN` repo secrets) → build + push.
   - Tag scheme: `harbor.anatoly.dev/ankimcp/keycloak:<KC_VERSION>-rememberme<PLUGIN_VERSION>` plus `:latest`.
...
4. **README.md** — concise, explains:
   ...
   - That no secrets live in this repo (Harbor token in Actions secrets only).
...
5. **Downstream coordination with `anki-mcp-infrastructure`** (separate repo, separate PR/commit there):
   - Edit `apps/keycloak/templates/keycloak.yaml` → set `spec.image: harbor.anatoly.dev/ankimcp/keycloak:<tag>` on the Keycloak CR.
...
- **Repo visibility: public or private?** Contents have no secrets (Harbor token lives in Actions secrets, never committed). Public has unlimited Actions minutes, fits AnkiMCP's mostly-public stack, and allows easier contribution-back to Herdo. Private is fine if there's an org-wide preference. **No security advantage either way for this content.**
- **Exact image tag scheme.** Proposed `<KC_VERSION>-rememberme<PLUGIN_VERSION>` (e.g., `26.5.7-rememberme1.0.0`). Alternatives: date-based (`26.5.7-2026.05.14`), commit-SHA-based (`26.5.7-rememberme-abc1234`). Pick one before CI lands.
...
The infrastructure repo `anki-mcp-infrastructure` deploys Keycloak via the Keycloak Operator and reconciles realm configuration via a `keycloak-config-cli` PostSync hook. After this image build lands and is pushed to Harbor, the infra repo's Keycloak `image` field plus realm JSON need the updates listed in "Pending work" item #5 above.
