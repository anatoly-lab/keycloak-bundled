...
1. **Vendor the plugin source** under `src/`. Clone Herdo's repo and copy the Java files, then:
   - Update `pom.xml` to target the current Keycloak BOM (currently 26.7.2; see "Example consumer: AnkiMCP" below for the rationale behind this specific pin).
...
**Keycloak version this repo tracks:** `26.7.2`. The consumer's Keycloak Operator is bumped to `26.7.2` in lockstep (this image is published first, then the operator follows — see the coordination rule below). The earlier `26.5.7` hold — motivated by `adorsys/keycloak-config-cli` lacking a `6.5.0-26.6.x` build — no longer blocks version bumps: config-cli stays pinned at its `6.5.1-26.5.5` tag and reconciles a newer server cleanly so long as the consumer's realm JSON uses none of the newer server's version-only realm fields. Verified for `26.6.4` and `26.7.2` (consumer realm JSON checked against config-cli `6.5.1-26.5.5`); re-check on each bump. This repo tracks the same Keycloak version the consumer deploys.
